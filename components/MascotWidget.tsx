...
export default function MascotWidget() {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-45 flex flex-col items-end gap-3 font-sans">
      {/* Speech Bubble Card */}
      {isOpen && (
        <div className="absolute bottom-20 right-0 sm:bottom-0 sm:right-24 w-[280px] sm:w-[320px] bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-2xl p-5 shadow-2xl animate-fade-in z-50">
          {/* Arrow pointing down on mobile, pointing right on desktop */}
          <div className="absolute -bottom-2 right-6 w-4 h-4 bg-white dark:bg-zinc-900 border-r border-b border-zinc-200 dark:border-zinc-800 rotate-45 sm:hidden" />
          <div className="absolute bottom-6 -right-2 w-4 h-4 bg-white dark:bg-zinc-900 border-t border-r border-zinc-200 dark:border-zinc-800 rotate-45 hidden sm:block" />

          {/* Close button */}
          <button
            onClick={() => setIsOpen(false)}
            className="absolute top-3 right-3 text-zinc-400 hover:text-zinc-600 dark:hover:text-white transition-colors cursor-pointer"
          ...
          <div className="flex items-center gap-2 mb-2">
            <span className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-600 dark:text-emerald-450 text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full flex items-center gap-1">
          ...
          <p className="text-xs text-zinc-650 dark:text-zinc-450 mt-1.5 leading-relaxed">
